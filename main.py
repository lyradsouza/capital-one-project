...
# make dataframe
df = pd.DataFrame.from_dict(word_frequencies, orient = 'index').reset_index()
df = df.rename(columns = {"index": "word", 0: "frequencies"})
df = df.sort_values("frequencies", ascending=False).head(30)

max_frequency = max(word_frequencies.values())
word_frequencies_values = {}
for word in word_frequencies.keys():
    word_frequencies_values[word] = word_frequencies[word]/max_frequency
...
for sent in sent_token:
    ...
    for word in sentence:
        if word.lower() in word_frequencies_values.keys():
            if sent not in sentence_scores.keys():
                sentence_scores[sent] = word_frequencies_values[word.lower()]
            else:
                sentence_scores[sent] += word_frequencies_values[word.lower()]
...
result = run_summarization(article_content)
